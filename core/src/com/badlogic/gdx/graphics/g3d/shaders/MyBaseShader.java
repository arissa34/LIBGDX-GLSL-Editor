...
public abstract class MyBaseShader implements Shader {
    public interface Validator {
        /** @return True if the input is valid for the renderable, false otherwise. */
        boolean validate (final MyBaseShader shader, final int inputID, final Renderable renderable);
    }
    ...
    public interface Setter {
        ...
        boolean isGlobal (final MyBaseShader shader, final int inputID);

        void set (final MyBaseShader shader, final int inputID, final Renderable renderable, final Attributes combinedAttributes);
    }

    public abstract static class GlobalSetter implements Setter {
        @Override
        public boolean isGlobal (final MyBaseShader shader, final int inputID) {
            return true;
        }
    }
    ...
    public static class Uniform implements Validator {
        ...
        public boolean validate (final MyBaseShader shader, final int inputID, final Renderable renderable) {
    ...
    public final Array<Setter> setters = new Array<Setter>();
    ...
    public void init (final ShaderProgram program, final Renderable renderable) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (validator != null && !validator.validate(MyBaseShader.this, i, renderable))
                locations[i] = -1;
            else {
    ...
    public final int[] getAttributeLocations (final VertexAttributes attrs) {
        tempArray.clear();
